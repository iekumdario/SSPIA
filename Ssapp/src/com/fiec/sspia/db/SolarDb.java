...
public class SolarDb {
	...
	public String[] getDetails(int id) {
		...
		Cursor cursor = db.query(DBHelper.TABLEDETALLE, allColumns_detail,
				null, null, null, null, null);
		// Cursor cursor = db.query(DBHelper.TABLEDETALLE, allColumns_detail,
		// DBHelper.IDDETALLE+"="+id, null, null, null,null);
		cursor.move(id);
		for (int k = 0; k < 14; k++) {
		...
		/*
		 * while(!cursor.isAfterLast()){ aux[j] = cursor.getString(i);
		 * Log.w("gmaTag", "aux = "+aux[j]); j++; i++; cursor.moveToNext(); }
		 */
		cursor.close();
		return aux;
	}
